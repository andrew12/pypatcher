...
def hexlist(v):
    return ', '.join(hex(x) for x in v)

def printhex(prefix, lst):
    prefix += '['
    line = prefix
    for b in lst:
        s = hex(b) + ', '
...
for mod in mods:
    print('- name:', mod['name'])
    print('  shortname:', mod['shortname'])
    if 'type' in mod and mod['type'] == 'union':
        print('  type: union')
        print('  offset:', hex(mod['offset']))
        print('  patches:')
        for patch in mod['patches']:
            print('  - name:', patch['name'])
            print('    shortname:', patch['shortname'])
            printhex('    patch: ', patch['patch'])
    else:
        print('  patches:')
        for patch in mod['patches']:
            oneliner = '  - {offset: %s, off: [%s], on: [%s]}' % (hex(patch['offset']), hexlist(patch['off']), hexlist(patch['on']))
            if len(oneliner) < 100:
                print(oneliner)
            else:
                print('  - offset:', hex(patch['offset']))
                printhex('    off: ', patch['off'])
                printhex('    on:  ', patch['on'])
